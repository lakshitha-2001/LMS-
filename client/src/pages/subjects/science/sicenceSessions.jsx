  ...
  useEffect(() => {
    const fetchSessions = async () => {
      try {
        ...
        // Console log to debug API data
        console.log("Sessions from API:", data)

        const filteredSessions = data.filter(
          (session) => session.subject.toLowerCase().replace(/\s+/g, '-') === (subjectId || "").toLowerCase()
        )

        // Console log to debug filtered sessions
        console.log(`Filtered sessions for subject ${subjectId}:`, filteredSessions)
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-violet-100">
      <div className="container mx-auto p-6 md:p-8">
        ...
        <div className="mb-8">
          <button
            onClick={() => navigate(-1)}
            className="group flex items-center gap-3 px-8 py-4 mb-8 bg-gradient-to-r from-blue-600 via-indigo-600 to-violet-600 hover:from-blue-700 hover:via-indigo-700 hover:to-violet-700 text-white font-bold rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:scale-[1.05] border border-blue-500/20"
          >
          ...
          <div className="text-center mb-8">
            <h1 className="text-4xl md:text-5xl font-bold mb-4 bg-gradient-to-r from-blue-700 via-indigo-600 to-violet-700 bg-clip-text text-transparent">
        ...
        {loading ? (
              ...
              >
                ...
                <div className="mb-4">
                  <div className="flex items-center justify-between mb-3">
                    <h2 className="text-xl font-bold text-slate-800 group-hover:bg-gradient-to-r group-hover:from-blue-600 group-hover:to-violet-600 group-hover:bg-clip-text group-hover:text-transparent transition-all duration-300">
                      {session.subject}
                    </h2>
                    <span className="bg-gradient-to-r from-blue-100 to-violet-100 text-indigo-700 px-3 py-1 rounded-full text-sm font-medium border border-indigo-200">
                      Live
                    </span>
                  </div>
                ...
                {/* Teacher Info */}
                <div className="flex items-center gap-3 p-4 bg-gradient-to-r from-blue-50 to-violet-50 rounded-xl mb-6 border border-indigo-100">
                  <img
                    src={
                      session.teacher?.img ||
                ...
                {session.link && (
                  <div className="space-y-3">
                    <button
                      ...
                      className="w-full bg-gradient-to-r from-blue-600 via-indigo-600 to-violet-600 hover:from-blue-700 hover:via-indigo-700 hover:to-violet-700 text-white font-semibold py-3 px-4 rounded-xl transition-all duration-300 transform hover:scale-[1.02] shadow-lg hover:shadow-xl"
                    >
                      {showLink[session._id] ? "Hide Session Link" : "Get Session Link"}
                    </button>

                    {showLink[session._id] && (
                      <div className="bg-gradient-to-r from-blue-50 to-violet-50 p-4 rounded-xl border border-indigo-200">
                        <div className="flex items-center justify-between bg-white p-3 rounded-lg border border-indigo-100 shadow-sm">
                          ...
                          <div className="flex space-x-2">
                            <button
                              ...
                              className="p-2 text-indigo-600 hover:text-indigo-800 hover:bg-indigo-50 rounded-lg transition-all duration-200 hover:scale-110"
                              title="Copy Link"
                            >
                            ...
                            <a
                              ...
                              className="p-2 text-violet-600 hover:text-violet-800 hover:bg-violet-50 rounded-lg transition-all duration-200 hover:scale-110"
                              title="Join Session"
                            >
          ...
          <div className="text-center py-16">
            <div className="bg-white/90 backdrop-blur-sm rounded-2xl p-8 max-w-md mx-auto shadow-lg border border-white/30">
              ...
              <button
                onClick={() => navigate(-1)}
                className="bg-gradient-to-r from-blue-600 to-violet-600 hover:from-blue-700 hover:to-violet-700 text-white px-6 py-3 rounded-xl transition-all duration-300 transform hover:scale-[1.02] shadow-lg hover:shadow-xl font-semibold"
              >
...
export default ScienceSessions